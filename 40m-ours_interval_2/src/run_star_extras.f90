         ...
         !   LEdd = (4.0d0 * pi * G * c * s%m(i) ) / s%opacity(i)
           ...
           do i = 1, s%nz               
               if (s% opacity(i) > 0.0d0) then
                  ...
                  !ratio = s% gradT(i) * 4.0d0 * (s % T(i) ** 4) * a / (3 * s% Peos(i) * s% extra_opacity_factor(i));
                  ! if (s% extra_opacity_factor(i) < 1) then
                  !    print *, '>>> ', s% extra_opacity_factor(i);
                  ! end if
                  
                  !ratio = (s% L(i) / ((4.0d0 * pi * G * c * s%m(i) ) / s%opacity(i))) / s% extra_opacity_factor(i)
                  ...
                  !         print *,'>>> ', s% extra_opacity_factor(i)
                  !       end if
                        s% extra_opacity_factor(i) = (1 / (1 + ratio - threshold)) ** (0.5d0)
                  end if 
               end if
           end do
