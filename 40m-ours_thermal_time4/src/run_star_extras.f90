...
! ***********************************************************************
      ...
      real, dimension(:), pointer :: grav_energy_memory => null()
      ...
      subroutine other_opacity_factor(id, ierr)
           ...
           integer :: i, j
           real(8) :: LEdd, ratio, opacity_target, kh
           ...
           !test
           real(dp), parameter :: Gc = 6.67430d-8    ! cgs
            real(dp), parameter :: L_sun = 3.828d33   ! erg/s
            real(dp) :: mass, Lphot_cgs, m_enc, r_enc, dm
           ...
           s% extra_opacity_factor(1:s% nz) = s% opacity_factor
            ...
            if (.not. associated(extra_opacity_factor_memory)) then
               ...
               allocate(grav_energy_memory(10000))
               extra_opacity_factor_memory = 1.0
               grav_energy_memory = 0
            end if

            Lphot_cgs = s%L_phot * L_sun
           ...
           do i = 1, s%nz               
               if (s% opacity(i) > 0.0d0) then
                  ...
                  if ( ratio > threshold) then
                     ...
                     !    !   if (r_enc > 0d0) then
                     !    !    mass = mass - Gc * m_enc / r_enc * dm
                     !    ! end if
                     !    mass = mass + s% dm(i)
                     ! end do
                     ! kh = G*M*M/(R*L)
                     ! consider using cgrav(1) insteaf of G
                     
                     ! secyer amount of seconds in a year
                     ! new option
                     kh = 0.75d0 * G * s% m(i) * s% m(i) / (s% r(s%nz) * s%L(i))
                     ! options we aggreed upon
                     !kh = 0.75d0 * G * mass * mass / (s% r(s%nz) * s% L_phot)

                     s% extra_opacity_factor(i) = opacity_target + EXP(- ABS(s% dt_years / (kh * secyer))) * (1 - opacity_target)
